...
const metadataPath = path.resolve("test-output/quit-clean-check.json");

await fs.rm(metadataPath, { force: true });

const runProcess = spawn(
  "npx",
  ["wdio", "run", "./wdio.conf.mts", "--spec", "./test/specs/quit-clean.e2e.ts"],
  {
    cwd: process.cwd(),
    stdio: "inherit",
...
if (remainingRows.length > 0) {
  ...
  throw new Error(`Clean quit verification failed: Obsidian processes still running after 5s: ${remainingSummary}${typeof exitCode === "number" ? `; WDIO exited with code ${exitCode}` : ""}`);
}

const exitCode = await waitForChildExit(runProcess, 20000);

console.log(`Verified clean quit for Obsidian app PID ${metadata.appPid}`);
console.log(`Verified clean quit for Obsidian renderer PID ${metadata.rendererPid}`);
